const MONITORING_STACKS=[]; // Opswork Stack that needed to monitored
// Function for posting to Slack
function postToSlack(messageText) {
    const https = require('https');
    const url = require('url');
    const slack_url = 'https://hooks.slack.com/services/xxxxxxxx'; // target slack channel to notify
    ...
    opsworks.describeStacks(function(err, data) {
        if (err) {
            ...
            for (var i=0;i<stacks.length; i++) {
                ...
                if (MONITORING_STACKS.indexOf(stack.Name) != -1){
